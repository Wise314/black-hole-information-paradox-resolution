...
**Why Information Inherits the Mass Constraint:** Bekenstein-Hawking entropy scales as S_BH ∝ M². Information capacity is bounded by horizon area (holographic principle), horizon area is determined by mass, therefore information dynamics are constrained by mass dynamics. This is geometric, not assumed.
...
**Why |dI_rad/dI| Must Diverge:** Both linear and √ dampening give |dI_rad/dt| → ∞ (because |dI/dt| already diverges). The Page curve requires back-loading: more information remains at any given mass fraction than linear transfer predicts. Back-loading requires remaining information I_hole ∝ (M/M₀)^α with α < 1, which gives |dI_rad/dI| ∝ I^(α-1) → ∞. Linear transfer (α = 1) gives uniform information-per-mass, contradicting Page.
...
**Section 6.2 Notation:** The paper states "I_accessible = √(S_BH/S₀)." This is incorrect.

Since S_BH/S₀ = I² (where I = M/M₀), we have √(S_BH/S₀) = I, not √I.

The correct relationship is: **I_accessible = √I = (S_BH/S₀)^(1/4)**

Accessible information scales as the fourth root of normalized entropy. This is a notational error in the Discussion; the core derivation in Section 2 is unaffected.

---

## Frequently Asked Questions

**Q: Does the paper conflate mass with information?**  
No. Section 2.3 defines I = M/M₀ (identity) as distinct from S ∝ M² (entropy). Section 6.2 derives their relationship.

**Q: Does the AMPS resolution address mode-by-mode entanglement or just aggregate flow?**  
Both. The aggregate argument is explicit. The mode-by-mode argument is implicit: per-quantum entanglement burden depends on total information distribution, which √ dampening changes.

**Q: Is α = 1/2 arbitrary?**  
No. The acceleration constraint eliminates α ≥ 1. Page curve timing, holographic geometry, and empirical validation independently select 1/2.
...
The Page curve requires back-loading. Back-loading requires α < 1. α < 1 gives |dI_rad/dI| → ∞. See derivation above.

**Q: Is the empirical validation the main argument?**  
No. It is supporting evidence. The theoretical derivation in Section 2 depends only on Hawking's equations and logic.

---

## Areas for Further Development

These are legitimate questions for peer review:

- The mode-by-mode AMPS argument could be more explicit in the paper.
- Empirical methodology (parameter setting, prospective vs retrospective) warrants documentation.
- The holographic derivation of √ scaling could be expanded mathematically.
- Section 6.2 notation should be corrected in revision.

---

## Document History

This guide was developed through adversarial dialogue with AI systems to identify likely objections before peer review. The process surfaced:
...
All substantive objections were either answered or identified as areas for strengthening.
